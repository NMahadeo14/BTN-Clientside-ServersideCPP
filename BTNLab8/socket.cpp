...
data_packet create_data_packet()
{
    ...
    std::cout << "Enter a data packet id: ";
    std::cin >> my_data_packet.id;
    std::cout << "Enter a data packet name: ";
    std::cin >> my_data_packet.name;
    std::cout << "What is the size if the data packet? ";
...
serialized_packet data_packet_serializer(data_packet mdp)
{
    char* msp = new char[16 * sizeof(char) + 1 * sizeof(bool) + 1 * sizeof(int) + mdp.size * sizeof(char)];
    char* auxptr = msp;

    memcpy(auxptr, &mdp.id, 1 * sizeof(int));
    auxptr += 1 * sizeof(int);

    memcpy(auxptr, mdp.name, 16 * sizeof(char));
    auxptr += 16 * sizeof(char);

    memcpy(auxptr, &mdp.size, 1 * sizeof(int));
    auxptr += 1 * sizeof(int);

    memcpy(auxptr, &mdp.tail, 1 * sizeof(int));
    auxptr += 1 * sizeof(int);

    memcpy(auxptr, mdp.numbers, mdp.size * sizeof(int));

    serialized_packet output;
    output.data = msp;
    output.length = 16 * sizeof(char) + 1 * sizeof(int) + 1 * sizeof(int) + 1 * sizeof(int) + mdp.size * sizeof(int);

    return output;
}

data_packet data_packet_deserializer(char* serialized_data_packet)
{
    data_packet deserialized_data_packet;
    char* auxptr = serialized_data_packet;

    memcpy(deserialized_data_packet.name, auxptr, 16 * sizeof(char));
    auxptr += 16 * sizeof(char);

    memcpy(&deserialized_data_packet.id, auxptr, 1 * sizeof(int));
    auxptr += 1 * sizeof(int);

    memcpy(&deserialized_data_packet.size, auxptr, 1 * sizeof(int));
    auxptr += 1 * sizeof(int);

    memcpy(&deserialized_data_packet.tail, auxptr, 1 * sizeof(int));
    auxptr += 1 * sizeof(int);
...
{
    struct serialized_packet serializedPack = data_packet_serializer(message);
    return send(this->active_socket, serializedPack.data, serializedPack.length, 0);
}
...
int Server_TCP::receive_data_packet(data_packet& thePacket)
{
    char RxBuffer[MAX_BUFFER_SIZE] = {};
    memset(RxBuffer, 0, MAX_BUFFER_SIZE);
    int num_bytes = recv(this->client_socket, RxBuffer, MAX_BUFFER_SIZE, 0);
    thePacket = data_packet_deserializer(RxBuffer);
    return num_bytes;
}
...
int Client_TCP::receive_data_packet(data_packet& thePacket)
{
    char RxBuffer[MAX_BUFFER_SIZE] = {};
    memset(RxBuffer, 0, MAX_BUFFER_SIZE);
    int num_bytes = recv(this->active_socket, RxBuffer, MAX_BUFFER_SIZE, 0);
