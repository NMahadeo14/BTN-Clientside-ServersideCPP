...
serialized_packet packet_serializer(packet my_packet) {
    ...
    char* my_serialized_packet = new char[16 * sizeof(char) + 1 * sizeof(bool) + 1 * sizeof(int) + my_packet.size * sizeof(char)];
...
{
    char* my_serialized_packet = new char[sizeof(char) + 1 * sizeof(unsigned int) + my_packet.size * sizeof(char)];
    char* auxptr = my_serialized_packet;

    memcpy(auxptr, &my_packet.number, 8 * sizeof(unsigned char));
    auxptr += 8 * sizeof(unsigned char);
    memcpy(auxptr, &my_packet.size, 1 * sizeof(unsigned int));
    auxptr += 1 * sizeof(unsigned int);
    memcpy(auxptr, my_packet.data, my_packet.size * sizeof(char));

    serialized_packet output;
    output.data = my_serialized_packet;
    output.length = 8 * sizeof(unsigned char) + 1 * sizeof(unsigned int) + my_packet.size * sizeof(char);
    return output;
}

new_packet new_packet_deserializer(char* s_packet)
{
    new_packet d_packet;
    char* auxptr = s_packet;

    memcpy(&d_packet.number, auxptr, 8 * sizeof(char));
    auxptr += 8 * sizeof(char);
    memcpy(&d_packet.size, auxptr, 1 * sizeof(unsigned int));
    auxptr += 1 * sizeof(unsigned int);
    d_packet.data = new char[d_packet.size];
    memcpy(d_packet.data, auxptr, sizeof(char) * d_packet.size);
    return d_packet;
}
...
int Server_TCP::receive_new_packet(new_packet& my_packet)
{
    char RxBuffer[MAX_BUFFER_SIZE] = {};
    memset(RxBuffer, 0, MAX_BUFFER_SIZE);
    int num_bytes = recv(this->client_socket, RxBuffer, MAX_BUFFER_SIZE, 0);
    my_packet = new_packet_deserializer(RxBuffer);
    return num_bytes;
}
...
int Client_TCP::receive_new_packet(new_packet& my_packet)
{
    char RxBuffer[MAX_BUFFER_SIZE] = {};
    memset(RxBuffer, 0, MAX_BUFFER_SIZE);
    int num_bytes = recv(this->active_socket, RxBuffer, MAX_BUFFER_SIZE, 0);
