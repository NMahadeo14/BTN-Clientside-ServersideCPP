...
bool Server_TCP::bind_socket() 
{
	...
	if (bind(this->active_socket, (struct sockaddr*) & SvrAddr, sizeof(SvrAddr)) == SOCKET_ERROR) 
	{
...
int Server_UDP::send_message(std::string msg, std::string ip, int port)
{
	this->ip = ip;
	this->port = port;
	return sendto(this->active_socket, msg.c_str(), sizeof(msg), 0, (struct sockaddr*) & this->port, sizeof(this->port));
}

sockaddr_in Server_UDP::receive_message(std::string& msg)
{
	char RxBuffer[MAX_BUFFER_SIZE] = {};
	memset(RxBuffer, 0, MAX_BUFFER_SIZE);
	struct sockaddr_in SerAddr;
	int addr_len = sizeof(SerAddr);
	recvfrom(this->active_socket, RxBuffer, sizeof(RxBuffer), 0, (struct sockaddr*) & SerAddr, &addr_len);
	msg = RxBuffer;
...
int Client_UDP::send_message(std::string msg, std::string ip, int port)
{
	this->ip = ip;
	this->port = port;
	return sendto(this->active_socket, msg.c_str(), sizeof(msg), 0, (struct sockaddr*) & this->port, sizeof(this->port));
...
sockaddr_in Client_UDP::receive_message(std::string& msg)
{
	char RxBuffer[MAX_BUFFER_SIZE] = {};
	memset(RxBuffer, 0, MAX_BUFFER_SIZE);
	struct sockaddr_in CliAddr;
	int addr_len = sizeof(CliAddr);
	recvfrom(this->active_socket, RxBuffer, sizeof(RxBuffer), 0, (struct sockaddr*) & CliAddr, &addr_len);
	msg = RxBuffer;
